...
一个输入框，把同一个问题**同时发给 10 个 AI 网页版**，收集各家回复，再自动汇总成一份横向总结。

| | | | |
|---|---|---|---|
| 千问 | 豆包 | 元宝 | 智谱 |
| Kimi | DeepSeek | MiniMax | 文心 | MiMo | 阶跃 |

**不需要申请任何 API Key**——复用你浏览器里已登录的各家网页版账号，通过网页自动化模拟真人提问、轮询抓取回复。多家独立作答、交叉验证，缓解单一模型的幻觉与视角偏差。输入框旁 📎 可挂 Word/PDF/MD/TXT 附件，发送时自动分发给各家（9 家入口实测接入，上传失败的家自动纯文本）。
